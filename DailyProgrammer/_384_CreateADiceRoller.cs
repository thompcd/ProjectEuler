...
{
    public class _384_CreateADiceRoller : Challenge
    {
        #region Challenge Prompt
        /*
         * [2018-06-18] Challenge #364 [Easy] Create a Dice Roller
            ...
            One major part of making character sheets is rolling the character's stats. Sadly, I have lost all my dice, so I'm asking for your help to make a dice roller for me to use!
            ...
            Also, just in case you didn't know, in D&D, not all the dice we roll are the normal cubes. A d6 is a cube, 
            because it's a 6-sided die, but a d20 has twenty sides, so it looks a lot closer to a ball than a cube.
            ...
            In my opinion that's not very random. If you run your code 3+ times with the same inputs and it gives the same outputs, 
            that wouldn't be super useful for a game of D&D, would it? If that happens with your code, try to find a way around that. 
            I'm guessing for some of the newer folks, this might be one of the trickier parts to get correct.
            
            Don't just multiply your roll by the number of dice, please. I don't know if any of you were thinking about doing that, but I was. 
            The problem is that if you do that, it eliminates a lot of possible values. For example,
            there's no way to roll 14 from 3d6 if you just roll it once and multiply by 3. Setting up a loop to roll each die is probably your best bet here.
        ...
        #region Fields
        string _rollInput = String.Empty;
        #endregion
        ...
        public override string Solve()
        {
            ...
            do
            {
                if (tryCounter > 0)
                {
                    Console.WriteLine("Invalid input. Word cannot have spaces");
                }
                inputString = PromptWithValidation("Enter a number of dice to roll with their number of faces.\n" +
                    "Use the format \"((int)numOfDice)d((int)numOfFaces)\"\n" +
                    "An example input is \"5d2\"");
                tryCounter++;
                //not much validation... but make sure we won't have nulls or spaces	
            } while (string.IsNullOrWhiteSpace(inputString) && !inputString.Any(Char.IsWhiteSpace));

            //5d12
            var rng = new Random();
            int rollSum = 0;

            var rollVals = inputString.Split('d');
            int.TryParse(rollVals[0], out int diceCount);
            if (diceCount < 1 || diceCount > 100)
            {
                return "Give a better number for teh dices.";
            }
            int.TryParse(rollVals[1], out int diceFace);
            if (diceCount < 2 || diceCount > 100)
            {
                return "You ain't got no dice with them many sides.";
            }
            for (int i = 0; i < diceCount; i++)
            {
                //if we roll a 0, reroll.
                int randomValue = 0;
                do
                {
                    //must offset +1, function is not inclusive of max value
                    randomValue = rng.Next(diceFace + 1);
                } while (randomValue == 0);
                rollSum += randomValue;
            }

            return rollSum.ToString();
        }
        public static string PromptWithValidation(string promptMsg)
        {
